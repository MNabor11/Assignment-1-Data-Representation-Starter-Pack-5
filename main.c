...
int main() {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        char func[64], input1[64], input2[64], expected[100];
        int32_t num;

        if (sscanf(line, "%s %s %s", func, input1, expected) >= 3) {
            output[0] = '\0';
            ...
            if (strcmp(func, "oct_to_bin") == 0) {
                ...
                num = atoi(input1);
                to_sign_magnitude(num, output);
            } else if (strcmp(func, "to_ones_complement") == 0) {
                num = atoi(input1);
                to_ones_complement(num, output);
            } else if (strcmp(func, "to_twos_complement") == 0) {
                num = atoi(input1);
                to_twos_complement(num, output);
            } else {
                continue;
            }
            ...
            if (strcmp(output, expected) == 0) {
                printf("Test %d: %s(%s) -> Expected: \"%s\", Got: \"%s\" [PASS]\n",
                       test_num, func, input1, expected, output);
                passed++;
            } else {
